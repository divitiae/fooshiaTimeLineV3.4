...
{
    public class AddFundsFragment : Android.Support.V4.App.Fragment, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, IPaymentResultWithDataListener
    {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(p1?.PaymentId))
                {
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.Global.RazorPay(p1.PaymentId, "wallet", keyValues).ConfigureAwait(false);
                        if (apiStatus == 200)
                        {
        ...
        private async Task PayStack(string email)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializePayStack("wallet", keyValues);
                    if (apiStatus == 200)
                    {
        ...
        private async Task CashFree(string name, string email, string phone)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializeCashFree("wallet", AppSettings.CashFreeCurrency, ListUtils.SettingsSiteList?.CashfreeSecretKey ?? "", AppSettings.CashfreeMode, keyValues);
                    if (apiStatus == 200)
                    {
        ...
        private async Task PaySera()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializePaySera("wallet", keyValues);
                    if (apiStatus == 200)
                    {
                        if (respond is InitializePaymentObject result)
                        {
