            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(p1?.PaymentId))
                {
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.Global.RazorPay(p1.PaymentId, "upgrade", keyValues).ConfigureAwait(false);
                        if (apiStatus == 200)
                        {
                            RunOnUiThread(() =>
                            {
        ...
        private async Task PayStack(string email)
        {
            try
            {
                ...
                {  
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializePayStack(type, keyValues);
                    if (apiStatus == 200)
                    {
        ...
        private async Task CashFree(string name, string email, string phone)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializeCashFree(type, AppSettings.CashFreeCurrency, ListUtils.SettingsSiteList?.CashfreeSecretKey ?? "", AppSettings.CashfreeMode, keyValues);
                    if (apiStatus == 200)
                    {
        ...
        private async Task PaySera()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializePaySera(type, new Dictionary<string, string>());
                    if (apiStatus == 200)
                    {
                        if (respond is InitializePaymentObject result)
                        {
