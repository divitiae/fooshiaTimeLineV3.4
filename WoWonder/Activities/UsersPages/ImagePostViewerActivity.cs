...
namespace WoWonder.Activities.UsersPages
{
    ...
    public class ImagePostViewerActivity : BaseActivity 
    {
        ...
        private void Copy_OnClick()
        {
            try
            {
                Methods.CopyToClipboard(this, AlbumData.Url); 
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
        ...
        private void BtnWonderOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                TxtCountWoWonder.Text = Methods.FunString.FormatPriceValue(Convert.ToInt32(AlbumData.PostWonders));
        ...
        private void Get_DataImage()
        {
            try
            {
                ...
                if (AlbumData != null)
                {
                    ...
                    if (index == "00")
                    { 
                        if (AlbumData.PhotoAlbum?.Count > 0)
                        {
                            var imagesList = AlbumData.PhotoAlbum;
                    ...
                    {
                        var imageData = AlbumData.PhotoAlbum[Convert.ToInt32(index)];
                        if (imageData != null)
                        {
                    ...
                    if (AppSettings.PostButton == PostButtonSystem.ReactionDefault || AppSettings.PostButton == PostButtonSystem.ReactionSubShine)
                    {
                        ...
                        if (AlbumData.Reaction.IsReacted != null && AlbumData.Reaction.IsReacted.Value)
                        {
                            if (!string.IsNullOrEmpty(AlbumData.Reaction.Type))
                            {
                                var react = ListUtils.SettingsSiteList?.PostReactionsTypes?.FirstOrDefault(a => a.Value?.Id == AlbumData.Reaction.Type).Value?.Id ?? "";
                                switch (react)
                                {
                                    case "1":
                    ...
                    else 
                    {
                        ...
                        TxtCountLike.Text = Methods.FunString.FormatPriceValue(Convert.ToInt32(AlbumData.PostLikes));
