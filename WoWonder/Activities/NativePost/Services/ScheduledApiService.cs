...
namespace WoWonder.Activities.NativePost.Services
{
    ...
    public class ScheduledApiService : Service
    {
        ...
        public override void OnCreate()
        {
            ...
            {
                base.OnCreate();
                PostUpdater = new PostUpdaterHelper(new Handler());
        ...
        public override StartCommandResult OnStartCommand(Intent intent, StartCommandFlags flags, int startId)
        {
            ...
            try
            {
                MainHandler?.PostDelayed(new PostUpdaterHelper(new Handler()), AppSettings.RefreshPostSeconds);
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
        ...
        public async void Run()
        {
            ...
            if (Methods.AppLifecycleObserver.AppState == "Background")
            {
                try
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Posts.GetGlobalPost(AppSettings.PostApiLimitOnBackground);
                    dbDatabase.Dispose();
