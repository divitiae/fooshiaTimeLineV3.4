        ...
        private async Task CashFree(CashFreeGetStatusObject statusObject, string request)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.CashFree(request, keyValues);
                    if (apiStatus == 200)
                    {
            ...
            public override async void OnPageStarted(WebView view, string url, Bitmap favicon)
            {
                ...
                { 
                    if (url.Contains("requests.php?f=cashfree"))
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.Global.CashFreeGetStatus(MActivity.CashFreeObject.AppId, ListUtils.SettingsSiteList?.CashfreeSecretKey ?? "", MActivity.CashFreeObject.OrderId, AppSettings.CashfreeMode);
                        if (apiStatus == 200)
                        {
